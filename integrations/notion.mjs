...
function normalizeDatabaseId(value) {
  ...
  let candidate = raw;
  try { if (/^https?:\/\//i.test(raw)) candidate = new URL(raw).pathname.split('/').filter(Boolean).pop() || raw; } catch { /* fall back to plain ID parsing */ }
  candidate = candidate.replace(/[-\s]/g, '').replace(/[^a-f0-9]/gi, '');
  return (candidate.match(/[a-f0-9]{32}/i)?.[0] || candidate).toLowerCase();
}
const databaseId = normalizeDatabaseId(setting('NOTION_DATABASE_ID'));
